...
echo "SERVER_FINGERPRINT: $SERVER_FINGERPRINT"
...
if [ "$SERVER_FINGERPRINT" = "$OFFICIAL_FINGERPRINT" ]; then
    ...
    cat github_rsa.pub >> ~/.ssh/known_hosts
    ...
    echo "WARNING: The server's SSH key fingerprint doesn't match the official GitHub fingerprint." 
