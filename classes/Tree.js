...
class Tree {
  ...
  async getLinkedPages() {
    try {
      ...
      const directoryCollections = navItems.map((item) => {
        ...
        if (item.sublinks) {
          return {
            ...
            title: item.title,
            children: item.sublinks.map((item) =>
              Object.assign(item, { type: "page" })
            ),
          }
        }
        return item
       ...
       */
      this.directory = await Bluebird.map(
  ...
  async getUnlinkedPages() {
    ...
    if (!this.navHasResources) {
      const resourceRoomName = await new ResourceRoom(
        this.accessToken,
        this.siteName
      ).get()
      unlinkedArr.push({
    ...
    const repoCollections = await new Collection(
      this.accessToken,
      this.siteName
    ).list()
    const unlinkedCollections = _.differenceBy(
      repoCollections,
      this.collections
    )
    ...
    if (unlinkedCollections.length > 0) {
      ...
      const unlinkedCollectionsToAdd = unlinkedCollections.map(
        (collection) => ({
          type: "collection",
